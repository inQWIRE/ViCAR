...
Lemma blah2 : forall (n m o p q : CC)
    (zx0 : n ~> m) (zx2 : (m × o) ~> p) (zx1 : o ~> o),
    ((zx0 ∘ id_ m) ⊗ (id_ o ∘ zx1)) ∘ zx2 ≃
    zx0 ⊗ zx1 ∘ zx2.
Proof.
intros.
...
Lemma assoc_rw_example {A B C M N : CC}
  ...
  f ⊗ g ∘ β_  M, N ≃ β_ B, C ∘ g ⊗ f.
Proof.  
  apply braiding_natural.
Qed.
...
Lemma assoc_rw_example {A B C M N : CC}
  ...
  ≃ (j ∘ g) ⊗ (h ∘ f).
Proof. 
  pose proof (braiding_natural f g) as H.
  let T := type of H in 
  assert T. 1:{
    exact H.
  }
...
Lemma assoc_lunit_viz_alt {A B C M N P : CC}
  (f : A ~> M) (g : B ~> N) (h : C ~> P) :
  ρ_ A ^-1 ⊗ g ∘ α_ A, I, N ≃ id_ A ⊗ (g ∘ λ_ N ^-1).
Proof.
Abort.
