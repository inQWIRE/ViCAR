...
Proof.
    ...
    - cleanup_zx.
      ...
      simpl_casts.
      bundle_wires.
      rewrite <- (cast_n_cup_unswapped (n + 0) n (Nat.add_0_r _) eq_refl).
      rewrite <- (nwire_removal_l (n_cup_unswapped _)) at 1.
      rewrite cast_compose_distribute, cast_id_eq.
      apply compose_simplify; [|easy].
      prop_perm_eq. *)
...
Proof.
    ...
    simpl_casts.
    bundle_wires; rewrite nwire_removal_r.
...
Lemma perm_of_n_swap : forall n,
    perm_of_zx (n_swap n) = fun k => if n <=? k then k else (n - 1 - k)%nat.
Proof.
    intros n.
    induction n.
    - easy.
...
(* Lemma n_cup_unswapped_n_swap_n_swap_helper : forall n {prf1 prf2 prf3 prf4 prf5 prf6},
    (— ↕ n_swap n ↕ (cast (S n) (S n) prf5 prf6 (n_swap n ↕ — ))
        ⟷ (cast (S n + S n) 2 prf1 prf2 (— ↕ n_cup_unswapped n ↕ —) ⟷ ⊃))
    ∝ cast _ _ prf3 prf4 (— ↕ (n_swap n ↕ n_swap n ⟷ n_cup_unswapped n) ↕ — 
        ⟷ ⊃).
Proof.
    intros.
    simpl_casts.
    rewrite stack_assoc', (stack_assoc —).
    simpl_casts.
    rewrite (cast_compose_l _ _ _ ⊃).
    rewrite <- cast_compose_mid_contract.
    simpl_casts.
    rewrite <- compose_assoc.
    apply compose_simplify; [|easy].
